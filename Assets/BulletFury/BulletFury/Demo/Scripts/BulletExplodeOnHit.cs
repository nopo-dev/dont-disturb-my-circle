...
namespace BulletFury.Demo
{
    public class BulletExplodeOnHit : MonoBehaviour
    {
        ...
        private IEnumerator PlayExplosion(Vector3 position)
        {
            ...
            var rndRotation = Quaternion.Euler(0f, 0f, Random.Range(0, 360));
            ...
            while (timer < timeToShrink)
            {
                ...
                Graphics.DrawMesh(mesh, Matrix4x4.TRS(position + rnd, rndRotation,
                    Vector3.Lerp(scale, Vector3.zero, t*t*t)), material, LayerMask.NameToLayer("Default"));    
